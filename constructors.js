...
let tristan = new User("Tristan", 24, "email", []) 
tristan.addSavedPost(5, "", 40)
////////// PROBLEM 6 //////////
...
User.prototype.removeSavedPost = function(id) {
  this.savedPosts = this.savedPosts.filter( (item) => {
    return item.id !== id; 
  }) 
}
tristan.removeSavedPost(5)
////////// PROBLEM 7 //////////
...
User.prototype.changePostRating = function(id, newRating) {
  this.savedPosts = this.savedPosts.map((item) => {
   if (item.id === id) {
